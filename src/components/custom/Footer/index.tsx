...
const Footer: React.FC = () => {
  return (
    <footer className="bg-fuchsia-950/30 text-fg-primary/90">
      <Link href="/" className="fixed right-4 lg:right-6 bottom-4 lg:bottom-6 h-8 w-8 bg-fg-primary/90 rounded-full indent-0 flex-center text-fg-secondary cursor-pointer font-bold">
        ...
        <div className="flex-gap-4 flex-col xl:col-span-2 md:col-span-2">
          <Link href="/" className="flex-gap-2 items-center">
            <Image
              src={logo}
              alt="logo"
              className="md:w-16 h-10  w-12 md:h-12 object-cover"
            />
            <h1 className="text-2xl md:text-3xl font-bold"> 3% CodeCraft </h1>
          </Link>
        ...
        </div>
         {
            usefulLinks.map((link, index) => (
              <div className="flex-gap-4  flex-col mt-1" key={index}>
                <h1 className="text-xl md:text-2xl font-semibold"> {link.mainHeading} </h1>
                {
                  link.links.map((iconLink, index ) => (
                    <Link href={iconLink.path} className="flex gap-3 hover:text-fg-secondary items-center font-semibold ease-transition" key={index}>
                      <LuCircleSlash />
